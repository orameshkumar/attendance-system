...
def find_best_known_match(face_emb, appearance, employees: dict, min_score=0.60):
    """
    Relaxed-threshold match against known (non-unknown, non-ignored) employees.
    Called as a fallback when match_person() returns None at strict thresholds.
    Ensures a person scoring >= 60% on face OR appearance is never created
    as a new unknown — they get recorded against the best-matching known employee.
...
def find_ignored_match(face_emb, appearance, employees: dict, min_score=0.60):
    """
    Check if the detected person/object matches any employee flagged is_ignored=True.
    If yes, the caller should silently skip this detection entirely.
    Returns (emp_id, score, method) or (None, 0.0, 'none').
    """
    best_id     = None
    best_score  = 0.0
